...
# -*- coding: utf-8 -*-
# ------------------------------------------------------------------------------
#  Name: createIAU2000_wkt_prj_files_v3.py
#  Author: Trent Hare
#  Original: Jan 2006
# Feb 2016:
...
#
# Note that create WKT projectsion can be tested using gdalsrsinfo, e.g.
# gdalsrsinfo "Mars 2000.prj"
#
#------------------------------------------------------------------------------
#
#
        ...
        for line in open(inputTable):
            ...
            if (tokens[2].replace(" ", "") != "") \
                ...
                if isInt(tokens[0]):  # is Naif number?
                    ...
                    # WKT
                    theStr = "GEOGCS[\"%s %s\",DATUM[\"D_%s_%s\",SPHEROID[\"%s_%s_%s\",%r,%r]],PRIMEM[\"Reference_Meridian\",0],UNIT[\"Degree\",0.0174532925199433],AUTHORITY[\"IAU%s\",\"%r\"]]" % ( 
                        theTarget, theYear, theTarget, theYear, theTarget, theYear, group, theA, flattening, theYear, gisCode)
