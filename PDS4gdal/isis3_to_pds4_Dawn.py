...
#/* -------------------------------------------------------------------- */
    ...
    theLine = '-co VAR_INVESTIGATION_AREA_LID_REFERENCE="urn:nasa:pds:context:instrument_host:spacecraft.dawn"\n'
    fileConfig.write(theLine)

    theLine = '-co VAR_INVESTIGATION_AREA_NAME="VESTA SCIENCE HAMO2 (VH2)"\n'
    fileConfig.write(theLine)
    ...
    try:
        observeID = isis3lbl['IsisCube']['Archive']['ObservationId']
        theLine = '-co VAR_OBSERVING_SYSTEM_NAME={}\n'.format(observeID)
        fileConfig.write(theLine)
    except KeyError:
        print('No ObservationId in ISIS3 Label')

    try:
        fileName = isis3lbl['IsisCube']['Archive']['FileName']
        theLine = '-co VAR_TITLE={}\n'.format(fileName)
        fileConfig.write(theLine)
    except KeyError:
        print('No FileName in ISIS3 Label')
    ...
    #try:
        #productID = isis3lbl['IsisCube']['Archive']['ProductId']
    #except KeyError:
