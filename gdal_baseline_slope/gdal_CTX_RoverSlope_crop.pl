...
###############################################################################
#
#_TITLE  gdal_CTX_slope.pl - uses gdaldem and imagemagick to create a 
#              slope image with legend and scale at 1/20 scale. 
...
#
#   gdal_CTX_slope.pl input_DEM.tif 
#
# Requirements:
#       GDAL Library (gdaldem)
#       requires composite from imagemagick
#
#_DESC make colorized slope at 1/20 scale with added legend and scalebar.
#
#_CALL  List of calls:
...
######################################################################
   if ($#ARGV < 0) {
      print " \n\n          *** HELP ***\n\n";
      print "gdal_CTX_slope.pl -  Create 8bit colorized rover baseline slope files with legends and scales, for CTX\n\n";
      print "Command line: \n";
      print "gdal_CTX_slope.pl input_dem.cub\n";
      print "  Only one baseline 5 for CTX is created, output files are automatically named\n\n";
      exit 1;
    }
    ...
    foreach (@baselines) {
      ...
      $rtmp = $root."xxxtemp_baseline_slope_20m.tif";
      $cmd = "gdalwarp -r average -tr 20 20 $outSlope $rtmp";
      print $cmd."\n";
      system($cmd);
  
      #colorize 
      $ctmp = $root."xxxtemp_baseline_clrslope_20m.png";
      ...
      $ltmp = $root."xxxtemp_baseline_clrslope_20m_legend.tif";
      $cmd = "composite -gravity NorthEast /usgs/cdev/contrib/bin/rover_Slope_legend.png $ctmp $ltmp";
      print $cmd."\n";
      system($cmd);

      #add existing scale bar (only for 20m/p file)
      $cmd = "composite -quality 92 -gravity SouthWest /usgs/cdev/contrib/bin/rover_HiRISE_scalebar_20mp.png $ltmp $outFigure";
      print $cmd."\n";
      system($cmd);
